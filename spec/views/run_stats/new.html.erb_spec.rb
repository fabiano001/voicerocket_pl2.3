...
RSpec.describe "run_stats/new", type: :view do
  before(:each) do
    assign(:run_stat, RunStat.new(
      :run => nil,
      :live_answer => 1,
      :machine_answer => 1,
      :do_not_call => 1,
  ...
  it "renders new run_stat form" do
    render

    assert_select "form[action=?][method=?]", run_stats_path, "post" do
